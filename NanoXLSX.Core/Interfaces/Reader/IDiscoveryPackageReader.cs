/*
 * NanoXLSX is a small .NET library to generate and read XLSX (Microsoft Excel 2007 or newer) files in an easy and native way
 * Copyright Raphael Stoeckli © 2026
 * This library is licensed under the MIT License.
 * You find a copy of the license in project folder or on: http://opensource.org/licenses/MIT
 */

using NanoXLSX.Internal;

namespace NanoXLSX.Interfaces.Reader
{
    /// <summary>
    /// Interface for queued package readers that are dispatched by discovered relationship type.
    /// </summary>
    internal interface IDiscoveryPackageReader : IPluginPackageReader, IDocumentReader
    {
        /// <summary>
        /// Gets or sets the relationship associated with the current reader execution.
        /// </summary>
        RelationshipInfo CurrentRelationship { get; set; }
    }
}
