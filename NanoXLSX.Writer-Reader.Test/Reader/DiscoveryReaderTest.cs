...
namespace NanoXLSX.Test.Writer_Reader.ReaderTest
{
    ...
    public class DiscoveryReaderTest : IDisposable
    {
        ...
        {
            PlugInLoader.InjectPlugins(new List<Type> { typeof(ReplacementDiscoveryReader) });
        ...
        {
            DiscoveryReader reader = new DiscoveryReader();
            IOException exception = Assert.Throws<IOException>(() => reader.Execute());
        ...
        public void TolerantValidationTest()
        {
            string relationships = RelationshipsXml(
                ...
                RelationshipXml("rId1", "http://example.test/relationships/type", "second.xml"), // Duplicate rId
                "<Relationship Id=\"rId2\" Type=\"http://example.test/relationships/type\" />"); // Missing target
        ...
        [MemberData(nameof(InvalidRelationshipCases))] // Nested test data
        public void TolerantInvalidEntryMatrixTest(string invalidRelationship, string expectedReason)
        {
            ...
            using (ZipArchive archive = new ZipArchive(stream, ZipArchiveMode.Read, true))
            {
                Workbook workbook = new Workbook("Sheet1");
                DiscoveryReader reader = new DiscoveryReader();
                reader.Init(archive, workbook, null); // no options = non-strict mode
        ...
        [Theory(DisplayName = "Structurally invalid relationship parts follow strict or tolerant validation")]
        [MemberData(nameof(InvalidRelationshipPartStructures))] // Nested test data
        public void InvalidRelationshipPartStructureTest(string invalidXml, string expectedDetail)
        {
            foreach (bool isStrict in new[] { false, true }) // test once with and without strict mode
            {
                using (MemoryStream stream = CreateRelationshipArchive(("_rels/.rels", invalidXml)))
                using (ZipArchive archive = new ZipArchive(stream, ZipArchiveMode.Read, true))
                {
                    Workbook workbook = new Workbook("Sheet1");
                    DiscoveryReader reader = new DiscoveryReader();
                    reader.Init(archive, workbook, new ReaderOptions { EnforceStrictValidation = isStrict });

                    if (isStrict)
                    {
                        IOException exception = Assert.Throws<IOException>(() => reader.Execute());
                        Assert.Contains("could not be parsed", exception.Message);
                        Assert.Contains(expectedDetail, exception.InnerException.Message);
                    }
                    else
                    {
                        reader.Execute();
                        RelationshipCatalog catalog = workbook.AuxiliaryData.GetData<RelationshipCatalog>(PlugInUUID.DiscoveryReader, PlugInUUID.DiscoveryCatalogEntity);
                        RelationshipDiscoveryIssue issue = Assert.Single(catalog.Issues);
                        Assert.Contains("could not be parsed", issue.Reason);
                        Assert.Empty(catalog.Relationships);
                    }
                }
            }
        ...
        [Theory(DisplayName = "Duplicate relationship-part ZIP entries follow strict or tolerant validation")]
        [InlineData(false)]
        [InlineData(true)]
        public void DuplicateRelationshipPartEntryTest(bool isStrict)
        {
            string relationships = RelationshipsXml(RelationshipXml("rId1", "http://example.test/type", "part.xml"));
            using (MemoryStream stream = CreateRelationshipArchive(
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void InvalidRelationshipPartPathTest(bool isStrict)
        {
        ...
        [Theory(DisplayName = "Invalid URI escapes in relationship-part paths follow strict or tolerant validation")]
        [InlineData(false)]
        [InlineData(true)]
        public void InvalidRelationshipPartUriEscapeTest(bool isStrict)
        {
            using (MemoryStream stream = CreateRelationshipArchive(("_rels/bad#.rels", RelationshipsXml(
                RelationshipXml("rId1", "http://example.test/type", "part.xml")))))
            using (ZipArchive archive = new ZipArchive(stream, ZipArchiveMode.Read, true))
            {
                Workbook workbook = new Workbook("Sheet1");
                ...
                else
                {
                    ...
                    RelationshipCatalog catalog = workbook.AuxiliaryData.GetData<RelationshipCatalog>(PlugInUUID.DiscoveryReader, PlugInUUID.DiscoveryCatalogEntity);
                    RelationshipDiscoveryIssue issue = Assert.Single(catalog.Issues);
                    Assert.Contains("relationship-part path:", issue.Reason);
                }
            }
        }
        ...
        [InlineData(false)]
        [InlineData(true)]
        public void MalformedOptionalPartIntegrationTest(bool isStrict)
        {
            ...
            {
                ReaderOptions options = new ReaderOptions { EnforceStrictValidation = isStrict };
                if (isStrict)
                {
                    Assert.Throws<IOException>(() => Extensions.WorkbookReader.Load(stream, options));
                }
                else
                {
                    Workbook workbook = Extensions.WorkbookReader.Load(stream, options);
        ...
        public void ExactDocumentTypeDispatchTest()
        {
            ...
            {
                PlugInLoader.InjectPlugins(new List<Type> { typeof(ExactTypeDiscoveryPackageReader) }); // Reader has the doc type of exactType
        ...
        [Fact(DisplayName = "XLSX reader rejects a discovery replacement that omits the catalog")]
        public void MissingDiscoveryCatalogTest()
        {
            PlugInLoader.InjectPlugins(new List<Type> { typeof(MissingCatalogDiscoveryReader) }); // Reader does not read a catalog
            using (MemoryStream stream = CreateWrittenWorkbookArchive())
            {
                IOException exception = Assert.Throws<IOException>(
                    () => Extensions.WorkbookReader.Load(stream));

                Assert.Contains("did not provide a relationship catalog", exception.Message);
            }
        }

        [Fact(DisplayName = "Relationship lookup skips candidates from the wrong source or target mode")]
        public void RelationshipLookupCompletesCandidateSearchTest()
        {
            const string documentType = "http://example.test/type";
            RelationshipCatalog catalog = new RelationshipCatalog();
            catalog.TryAdd(new RelationshipInfo("rIdExternal", documentType, "https://example.test", TargetMode.External, "_rels/.rels", "xl/workbook.xml", null));
            catalog.TryAdd(new RelationshipInfo("rIdWrongSource", documentType, "part.xml", TargetMode.Internal, "_rels/.rels", string.Empty, "part.xml"));
            RelationshipInfo expected = new RelationshipInfo("rIdExpected", documentType, "part.xml", TargetMode.Internal, "xl/_rels/workbook.xml.rels", "xl/workbook.xml", "xl/part.xml");
            catalog.TryAdd(expected);
            MethodInfo method = typeof(XlsxReader).GetMethod("GetRelationship", BindingFlags.NonPublic | BindingFlags.Static);

            RelationshipInfo actual = (RelationshipInfo)method.Invoke(null, new object[] { catalog, "xl/workbook.xml", documentType, false });
            RelationshipInfo optionalMissing = (RelationshipInfo)method.Invoke(null, new object[] { catalog, "missing.xml", documentType, false });

            Assert.Same(expected, actual);
            Assert.Null(optionalMissing);
        }

        [Theory(DisplayName = "Required ZIP entry lookup rejects empty and absent relationship targets")]
        [InlineData(null)]
        [InlineData("xl/missing.xml")]
        public void RequiredEntryMissingTest(string resolvedTargetPath)
        {
            ...
            MethodInfo method = typeof(XlsxReader).GetMethod("GetRequiredEntry", BindingFlags.NonPublic | BindingFlags.Static);

            TargetInvocationException invocationException = Assert.Throws<TargetInvocationException>(
                () => method.Invoke(null, new object[] { relationship, entries }));
            IOException exception = Assert.IsType<IOException>(invocationException.InnerException);

            Assert.Contains("was not found in the archive", exception.Message);
        }
        ...
        #region helperMethodsAndClasses
        ...
        {
            // Invalid relationship entry and partial issue text
            get
            {
        ...
        [ExcludeFromCodeCoverage]
        public static IEnumerable<object[]> InvalidRelationshipPartStructures // Must be public to be resolved
        {
            // Invalid relationship part entry and partial issue text
            get
            {
                yield return new object[] { "<Invalid xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\" />", "invalid root element" };
                yield return new object[]
                {
                    "<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Invalid /></Relationships>",
                    "invalid element"
                };
            }
        }
        ...
        [NanoXlsxPlugIn(PlugInUUID = PlugInUUID.DiscoveryReader, PlugInOrder = 100)]
        private class MissingCatalogDiscoveryReader : IDiscoveryReader
        {
            public Workbook Workbook { get; set; }
            public Interfaces.IOptions Options { get; set; }

            public void Init(ZipArchive archive, Workbook workbook, Interfaces.IOptions readerOptions)
            {
                Workbook = workbook;
        ...
        private class WorksheetDiscoveryPackageReader : IDiscoveryPackageReader
        {
            ...
            public Workbook Workbook { get; set; }
            public Interfaces.IOptions Options { get; set; }
            public Action<Stream, Workbook, string, Interfaces.IOptions, int?> InlinePluginHandler { get; set; }

            public void Init(Stream stream, Workbook workbook, Interfaces.IOptions options, Action<Stream, Workbook, string, Interfaces.IOptions, int?> inlinePluginHandler)
            {
